...
- **`MilaStoreReader` stays stateless and `Sendable`; `searchTranscripts` gets cheap by doing less, not by remembering.** Two things make the cost survivable and both are easy to undo by accident. `TranscriptFormatter.plainText` takes `fallback:` as an `@autoclosure`, so `namedTranscript` reads a recording's `.txt` sidecar only when NO segment carries a speaker label — hoisting that expression into a `let` puts one wasted file read back on every recording a search touches. And `sort: .createdAt` traverses in output order and stops at the `limit`-th hit; `sort: .relevance` cannot (ranking by match count has to score everything) and deliberately does not cap the candidate set, because losing the best hit silently is worse than a slow search. `MilaStoreReaderTests` pins both by counting sidecar reads through an internal `SidecarReader` seam — a discarded read is invisible in the results, so nothing else can see it. See #201.
